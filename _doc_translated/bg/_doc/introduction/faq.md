...
In short: these are non-realistic solutions today. We discuss this in further depth in our [Architecture Specification document](/bg/attachment/wiki/QubesArchitecture/arch-spec-0.3.pdf).
...
You can even [manually configure your encryption parameters](/bg/doc/encryption-config/), if you like!

### What do all these terms mean?

All Qubes-specific terms are defined in the [glossary](/bg/doc/glossary/).
...
We discuss this in much greater depth in our [Architecture Specification document](/bg/attachment/wiki/QubesArchitecture/arch-spec-0.3.pdf).

### How is Qubes affected by Xen Security Advisories (XSAs)?

See the [XSA Tracker](/bg/security/xsa/).
...
Please refer to [this page](/bg/doc/vm-sudo/).

### Why is dom0 so old?

Please see:
- [Why would one want to update software in dom0?](/bg/doc/software-update-dom0/#why-would-one-want-to-install-or-update-software-in-dom0)
- [Note on dom0 and EOL](/bg/doc/supported-versions/#note-on-dom0-and-eol)
...
Please see the [documentation guidelines](/bg/doc/doc-guidelines).
...
Instead, if you want to obtain your own, trustworthy copy of this website in a secure way, you should clone our [website repo](https://github.com/QubesOS/qubesos.github.io), [verify the PGP signatures on the commits and/or tags](/bg/security/verifying-signatures/#how-to-verify-qubes-repos) (signed by the [doc-signing keys](https://github.com/QubesOS/qubes-secpack/tree/master/keys/doc-signing)), then either [render the site on your local machine](https://github.com/QubesOS/qubesos.github.io/blob/master/README.md#instructions) or simply read the source, the vast majority of which was [intentionally written in Markdown so as to be readable as plain text for this very reason
...
See the [System Requirements](/bg/doc/system-requirements/).

### Is there a list of hardware that is compatible with Qubes OS?

See the [Hardware Compatibility List](/bg/hcl/).

### Is there any certified hardware for Qubes OS?

See [Certified Hardware](/bg/doc/certified-hardware/).
...
But if you understand the risk and accept it, read [documentation on multibooting](/bg/doc/multiboot/), 
it begins with an explanation of the risks with such a setup.

### Which version of Qubes am I running?

See [here](/bg/doc/version-scheme/#check-installed-version).
...
Please review the notes in the `qvm-prefs` man page and [here](/bg/doc/assigning-devices/) and be aware of the potential risks.
...
See also [here](/bg/doc/assigning-devices/).
...
2. [Enable the appropriate RPMFusion repos in the desired Fedora TemplateVM.](/bg/doc/software-update-vm/#rpmfusion-for-a-fedora-templatevm)
...
Although external media such as external hard drives or flash drives plugged in via USB are available in the USB qube, it is not recommended to access them directly from inside the USB qube. See [Block (Storage) Devices](/bg/doc/block-devices/) for more information.
...
The problem is that Qubes has [tens of thousands of different users](/bg/statistics/) with radically different needs and purposes.
...
Yes.
In general, the Qubes developers will not use a piece of software unless there is an *easy* way to verify both its **integrity** and **authenticity**, preferably via PGP signatures (see [Verifying Signatures](/bg/security/verifying-signatures/)).
Specifically:
 ...
 * If the software is security-sensitive and requires communication with the outside world, a "split" implementation is highly preferred (for examples, see [Split GPG](/bg/doc/split-gpg/) and [Split Bitcoin
 * If the software has dependencies, these should be packaged and available in repos for a [current, Qubes-supported version](/bg/doc/supported-versions/#templatevms) of Fedora (preferred) or Debian (unless all the insecure dependencies can run in an untrusted VM in a "split" implementation).
...
See [these instructions](/bg/doc/qubes-builder/).

### How do I submit a patch?

See the [Qubes Source Code Repositories](/bg/doc/source-code/) article.
...
Intel TXT (used in [Anti Evil Maid](/bg/doc/anti-evil-maid/)) at least tries to avoid or limit trust in BIOS.
...
Yes, Qubes natively supports automation via [Salt (SaltStack)](/bg/doc/salt/).
There is also the unofficial [ansible-qubes toolkit](https://github.com/Rudd-O/ansible-qubes).
(**Warning:** Since this is an external project that has not been reviewed or endorsed by the Qubes team, [allowing it to manage dom0 may be a security risk](/bg/doc/security-guidelines/#dom0-precautions).)