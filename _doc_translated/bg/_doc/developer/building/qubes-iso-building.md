...
redirect_from:
- /bg/doc/qubes-r3-building/
- /bg/doc/QubesR3Building/
- /bg/wiki/QubesR3Building/
ref: 203
...
Once you've built the development AppVM, open a Terminal window to it and install the necessary dependencies (see [QubesBuilder](/bg/doc/qubes-builder/) for more info):
...
See [Verifying Signatures](/bg/security/verifying-signatures/#1-get-the-qubes-master-signing-key-and-verify-its-authenticity) for verification sources.
...
Let's configure the builder first (see [procedure](/bg/doc/qubes-iso-building/#build-using-manual-steps) at bottom if you would prefer to manually configure):
...
See the related note under [Extra Whonix Build Options](/bg/doc/building-whonix-template/).

You may also want to add `COMPONENTS := $(filter-out gcc,$(COMPONENTS))` to bypass a multiple hour compile step.
See [QubesBuilder](/bg/doc/qubes-builder/#use-pre-built-qubes-packages) for more detail.
