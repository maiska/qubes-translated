...
redirect_from:
- /bg/doc/VMInterface/
- /bg/doc/SystemDoc/VMInterface/
- /bg/wiki/SystemDoc/VMInterface/
ref: 165
...
- `qubes.Backup` - store Qubes backup. The service receives location chosen by
  ...
  backup format](/bg/doc/BackupEmergencyRestoreV2/))
...
- `qubes.Filecopy` - receive some files from other VM. Files sent in [qfile format](/bg/doc/qfilecopy/)
- `qubes.OpenInVM` - open a file in called VM. Service receives a single file on stdin (in
  [qfile format](/bg/doc/qfilecopy/). After a file viewer/editor is terminated, if
...
- `qubes.Restore` - retrieve Qubes backup. The service receives backup location
  ...
  archive in [qfile format](/bg/doc/qfilecopy/) (core-agent-linux component contains
...
- `systemctl start qubes-update-check.timer` (and similarly stop) - called when enabling/disabling updates checking in given VM (`qubes-update-check` [qvm-service](/bg/doc/qubes-service/))
