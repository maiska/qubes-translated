...
See [Qubes Downloads](/bg/downloads/).
...
scratch and use [qubes backup and restore tools](/bg/doc/backup-restore/) for
migrating of all of the user VMs.

Users of Qubes R3.0 can upgrade using [experimental
procedure](/bg/doc/upgrade-to-r3.1/).

### From R2 or earlier

When upgrading from earlier versions the easiest and safest way is to install
it from scratch and use [qubes backup and restore tools](/bg/doc/backup-restore/)
for migrating of all of the user VMs.

Alternatively you can [upgrade to R3.0
using](/bg/doc/releases/3.0/release-notes/#upgrading) first, then follow the
instructions above. This will be time consuming process.

[salt-doc]: /bg/doc/salt/
[pvgrub-doc]: /bg/doc/managing-vm-kernel/
