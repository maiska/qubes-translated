...
redirect_from:
- /bg/doc/BackupRestore/
- /bg/wiki/BackupRestore/
ref: 87
...
3. Select the destination for the backup:
   ...
   If you wish to send your backup to a [USB mass storage device](/bg/doc/usb/), you can use the directory selection widget to mount a connected device (under "Other locations" item on the left); or first mount the device in a VM, then select the mount point inside that VM as the backup destination.
...
2. Select the source location of the backup to be restored:
   ...
   - If your backup is located on a [USB mass storage device](/bg/doc/usb/), attach it first to another VM or select `sys-usb` in the next item.
 ...
 * [Qubes R4 or newer](/bg/doc/backup-emergency-restore-v4/)
 * [Qubes R3](/bg/doc/backup-emergency-restore-v3/)
 * [Qubes R2 or older](/bg/doc/backup-emergency-restore-v2/)


Migrating Between Two Physical Machines
---------------------------------------

In order to migrate your Qubes system from one physical machine to another, simply follow the backup procedure on the old machine, [install Qubes](/bg/downloads/) on the new machine, and follow the restoration procedure on the new machine.
