...
Create a VM named win7new in [HVM](/bg/doc/hvm/) mode (Xen's current PVH limitations precludes from using PVH):
...
The VM is now ready to be started; the best practice is to use an installation ISO [located in a VM](/bg/doc/hvm/#installing-an-os-in-an-hvm-domain):
...
- on R3.2: see [this page](/bg/doc/windows-tools/)
...
Please see the [Customizing Windows 7 templates](/bg/doc/windows-template-customization/) page (despite the focus on preparing the VM for use as a template, most of the instructions are independent from how the VM will be used - ie. TemplateVM or StandaloneVM).